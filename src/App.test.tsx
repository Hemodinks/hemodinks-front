...
describe("App", () => {
  ...
  it("faz login, salva a sessao JWT e carrega usuarios", async () => {
    ...
    expect(
      screen.getByText("Administrador | gmarcone@gmail.com"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: /abrir usuários/i }),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      within(userRow).queryByText("+55 (81) 99999-9999"),
    ).not.toBeInTheDocument();
    expect(
      within(userRow).queryByText("529.982.247-25"),
    ).not.toBeInTheDocument();
    ...
    await user.click(
      within(userRow).getByLabelText("Contato de Ana Hemodinks"),
    );

    const contactDialog = screen.getByRole("dialog", { name: "Ana Hemodinks" });
    expect(
      within(contactDialog).getByText("ana@hemodinks.com"),
    ).toBeInTheDocument();
    expect(
      within(contactDialog).getByText("+55 (81) 99999-9999"),
    ).toBeInTheDocument();
    await user.keyboard("{Escape}");
    await waitFor(() =>
      expect(
        screen.queryByRole("dialog", { name: "Ana Hemodinks" }),
      ).not.toBeInTheDocument(),
    );
  ...
  it("libera menu administrativo completo e CRUD de clinicas para superadministrador", async () => {
    ...
    expect(
      within(sidebar).getByRole("button", { name: /usuários/i }),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      within(sidebar).getByRole("button", { name: /^faturamento$/i }),
    ).toBeInTheDocument();
    expect(
      within(sidebar).getByRole("button", { name: /^financeiro$/i }),
    ).toBeInTheDocument();
    expect(
      within(sidebar).getByRole("button", { name: /grupos médicos/i }),
    ).toBeInTheDocument();
    expect(
      within(sidebar).getByRole("button", { name: /^clínicas$/i }),
    ).toBeInTheDocument();
    ...
    expect(
      within(planSelect).getByRole("option", { name: "Completa" }),
    ).toBeInTheDocument();
    ...
    expect(screen.queryByLabelText("Trial ate")).not.toBeInTheDocument();
    expect(
      screen.getByRole("group", { name: "Módulos contratados" }),
    ).toBeInTheDocument();
  ...
  it("oculta no menu e no dashboard os modulos nao contratados ate para superadministrador", async () => {
    ...
    expect(
      within(sidebar).queryByRole("button", { name: /usuários/i }),
    ).not.toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByRole("button", { name: /abrir agenda/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("encerra a sessao quando a API sinaliza token expirado", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Painel inicial" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("nao exibe erro no dashboard quando o resumo retorna 403 para medico", async () => {
    ...
    expect(
      screen.queryByText(/request failed with status code 403/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("abre a agenda por URL direta", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Agenda e notificações" }),
    ).toBeInTheDocument();
  ...
  it("exclui apenas o evento clicado na agenda", async () => {
    ...
    await user.click(
      within(eventCard as HTMLElement).getByLabelText("Excluir"),
    );
    ...
    expect(
      within(confirmDialog).getByText(/Deseja excluir "Evento A"/i),
    ).toBeInTheDocument();
    expect(api.deleteAgendaEvent).not.toHaveBeenCalled();

    await user.click(
      within(confirmDialog).getByRole("button", { name: "Sim" }),
    );
  ...
  it("alterna entre tema claro e escuro no painel logado", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /abrir configuração do sistema/i }),
    );
    expect(
      await screen.findByRole("heading", {
  ...
  it("remove a edicao da marca das configuracoes do sistema", async () => {
    ...
    expect(
      screen.queryByLabelText("Nome exibido no sistema"),
    ).not.toBeInTheDocument();
    expect(screen.queryByLabelText("Foto da empresa")).not.toBeInTheDocument();
    expect(
      screen.getByRole("group", { name: "Tema do sistema" }),
    ).toBeInTheDocument();
  ...
  it("abre as notificacoes do usuario logado", async () => {
    ...
    expect(
      within(dialog).getByText("Procedimento: Consulta"),
    ).toBeInTheDocument();
  });
  ...
  it("destaca observacoes nao lidas na lista de pacientes e no modal", async () => {
    ...
    const observationButton = within(patientRow.closest("tr")!).getByRole(
      "button",
    ...
    expect(
      within(dialog).getByText("3 observações não lidas"),
    ).toBeInTheDocument();
  ...
  it("exibe hospital e destaque visual no popup de informacoes do paciente", async () => {
    ...
    expect(
      within(dialog).getByText("Santa Clara - Mater Dei"),
    ).toBeInTheDocument();
    ...
    expect(
      await within(dialog).findByText("Parcialmente recebido"),
    ).toBeInTheDocument();
  });
  ...
  it("carrega foto de perfil pela API e exibe iniciais se a imagem falhar", async () => {
    ...
    expect(
      screen.getByLabelText("Sem foto de George Marcone"),
    ).toBeInTheDocument();
  });
  ...
  it("bloqueia o primeiro acesso ate a troca da senha padrao", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Painel inicial" }),
    ).toBeInTheDocument();
  ...
  it("permite ao medico fechar o proprio cadastro e voltar ao painel", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: /pacientes/i }),
    ).toBeInTheDocument();
    await waitFor(() => {
  ...
  it("permite ao medico navegar pelo menu enquanto o proprio cadastro ainda carrega", async () => {
    ...
    const sidebar = screen.getByLabelText("Sessão ativa");
    await user.click(
      within(sidebar).getByRole("button", { name: /^painel$/i }),
    );
  ...
  it("abre painel para controller e permite cadastrar, editar e anexar arquivos de pacientes sem foto", async () => {
    ...
    expect(await screen.findByText("Paciente Hemodinks")).toBeInTheDocument();
    expect(
      await screen.findByRole("button", { name: /novo paciente/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /exportar xlsx/i }),
    ).toBeInTheDocument();
    expect(
  ...
  it("abre popup de informacoes, preenche o formulario ao editar e exclui usuario", async () => {
    ...
    await user.click(
      within(tableRow).getByLabelText("Detalhes de Ana Hemodinks"),
    );

    const infoDialog = screen.getByRole("dialog", { name: "Ana Hemodinks" });
    expect(infoDialog).toBeInTheDocument();
    expect(
      within(infoDialog).getByText("Data de nascimento"),
    ).toBeInTheDocument();
    ...
    await user.click(
      within(contactDialog).getByRole("button", { name: /copiar email/i }),
    );
    expect(
      await within(contactDialog).findByText("Copiado"),
    ).toBeInTheDocument();
    await user.click(screen.getByTitle("Fechar"));
    ...
    expect(
      within(confirmDialog).getByText(/Deseja excluir "Ana Hemodinks"/i),
    ).toBeInTheDocument();
