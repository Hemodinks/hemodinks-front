...
function receiptRows(data: GeneratedReceiptData) {
  return [
    ...
    ['Data do recebimento', new Date(data.paymentDate).toLocaleString('pt-BR')],
...
async function downloadPdf(data: GeneratedReceiptData) {
  ...
  document.text('Documento gerado eletronicamente pelo Hemodinks.', 18, Math.min(top + 10, 278));
  triggerDownload(document.output('blob'), fileName(data, 'pdf'));
}
...
async function downloadJpg(data: GeneratedReceiptData) {
  ...
  context.fillText('Documento gerado eletronicamente pelo Hemodinks.', 90, 1630);
