    ...
    >
      <div className="panel-title">
        <div>
          ...
          <h2 id="billing-account-title">{selectedAccount.numeroDocumento}</h2>
        </div>
        <IconButton
            ...
            <IconButton
              ...
              onClick={() =>
                setAccountDraft({
                  ...
                  dataVencimento: selectedAccount.dataVencimento.slice(0, 10),
      ...
      {accountDraft && (
        <form className="billing-filter-grid" onSubmit={saveAccount}>
          <TextField
            ...
            required
            onValueChange={(value) => setAccountDraft({ ...accountDraft, numeroDocumento: value })}
          />
          <TextField
            label="Descrição"
            value={accountDraft.descricao}
            required
            onValueChange={(value) => setAccountDraft({ ...accountDraft, descricao: value })}
          />
          <TextField
            label="Emissão"
            type="date"
            ...
            required
            onValueChange={(value) => setAccountDraft({ ...accountDraft, dataEmissao: value })}
          />
          <TextField
            label="Vencimento"
            type="date"
            ...
            required
            onValueChange={(value) => setAccountDraft({ ...accountDraft, dataVencimento: value })}
          />
          <TextField
            ...
            required
            onValueChange={(value) => setAccountDraft({ ...accountDraft, valorOriginal: value })}
          />
          <TextField
            ...
            required
            onValueChange={(value) => setAccountDraft({ ...accountDraft, valorAjustado: value })}
          />
          <TextField
            label="Observação"
            value={accountDraft.observacao}
            onValueChange={(value) => setAccountDraft({ ...accountDraft, observacao: value })}
          />
          ...
          <tbody>
            {selectedAccount.recebimentos.map((item) => (
              <tr key={item.id}>
                ...
                <td>{item.estornado ? `Estornado — ${item.motivoEstorno || ''}` : 'Ativo'}</td>
                <td>
