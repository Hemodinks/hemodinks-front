...
describe('BillingPage finance', () => {
  ...
  it('filtra contas e mostra cards calculados pelo backend', async () => {
    ...
    const filtersAccordion = screen.getByText('Filtros financeiros').closest('details');
    expect(filtersAccordion).not.toHaveAttribute('open');
    fireEvent.click(screen.getByText('Filtros financeiros'));
    expect(screen.getByLabelText('Buscar por documento ou paciente')).toHaveAttribute(
      'placeholder',
    ...
    expect(
      screen.getByRole('heading', { name: 'Registrar recebimento' }).closest('.data-panel'),
  ...
  it('lança pagamento parcial e exige motivo no estorno', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Confirmar estorno' })).toHaveClass(
      'billing-reversal-modal',
    );
    expect(
    ...
    await waitFor(() =>
      expect(services.estornarRecebimento).toHaveBeenCalledWith(1, 'Lançamento duplicado', 'token'),
    );
  });
  ...
  it('exibe o erro do recebimento ao lado do botão por 10 segundos', async () => {
    ...
    const receiptPanel = screen
      ...
      .closest('.data-panel');
    expect(receiptPanel).toHaveTextContent('Recebimento inválido ou superior ao saldo aberto.');
    ...
    expect(receiptPanel).not.toHaveTextContent('Recebimento inválido ou superior ao saldo aberto.');
    vi.useRealTimers();
  });
  ...
  it('permite escolher PDF ou JPG para o comprovante gerado e valida o anexo bancário', async () => {
    ...
    expect(fileInput).toHaveAttribute('accept', '.pdf,.jpg,.jpeg,application/pdf,image/jpeg');
    ...
    expect(
      screen.getAllByText('Selecione um comprovante bancário no formato PDF ou JPG.'),
    ).toHaveLength(2);
  });
  ...
  it('baixa o comprovante com a extensão correspondente ao conteúdo', async () => {
    ...
    const createObjectUrl = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:comprovante');
    ...
    try {
      ...
      await waitFor(() =>
        expect(services.downloadComprovanteRecebimento).toHaveBeenCalledWith(1, 'token'),
      );
  ...
  it('abre o detalhe da conta no modal largo', async () => {
    ...
    expect(dialog).toHaveClass('billing-wide-modal', 'billing-account-detail-modal');
    expect(dialog.parentElement).toHaveClass('modal-backdrop', 'billing-account-detail-backdrop');
  });
});
