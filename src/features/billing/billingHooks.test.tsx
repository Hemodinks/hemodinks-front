...
describe("billing domain hooks", () => {
  ...
  it("loads invoicing and delegates its mutation boundary", async () => {
    ...
    await result.current.changeInvoiceStatus(
      4,
      { status: "Enviado" } as never,
      token,
    );
  ...
  it("loads receivables, calculates totals and delegates finance operations", async () => {
    ...
    await result.current.registerReceipt(
      5,
      { valor: 20, formaRecebimento: "Pix" } as never,
      token,
    );
    await result.current.reverseReceipt(10, "Duplicado", token);
    await result.current.searchReceivables({ page: 2 } as never, token);
    await result.current.cancelReceivable(
      5,
      { motivo: "Cancelamento" } as never,
      token,
    );
    ...
    expect(services.cancelContaReceber).toHaveBeenCalledWith(
      5,
      { motivo: "Cancelamento" },
      token,
    );
  ...
  it("loads prices and selects create or update from the identifier", async () => {
    ...
    expect(services.updateConvenioProcedimentoPreco).toHaveBeenCalledWith(
      6,
      payload,
      token,
    );
    expect(services.deactivateConvenioProcedimentoPreco).toHaveBeenCalledWith(
      6,
      token,
    );
  });
  ...
  it("creates fresh initial form values", () => {
    ...
    expect(createInitialFaturamentoForm()).not.toBe(
      createInitialFaturamentoForm(),
    );
