...
export function NotificationsModal({
  ...
  return (
    <Modal titleId="notifications-title" className="notifications-modal" onClose={onClose}>
      ...
      {loading && (
        <AlertMessage type="success" icon={<Bell size={17} />}>
      ...
      {notifications.length ? (
        <ul className="notifications-list">
          {notifications.map((notification) => {
            ...
            return (
              <li
                key={`${notification.tipo}-${notification.id}`}
                className={notification.dataLeitura ? 'is-read' : 'is-unread'}
              >
                ...
                <div className="notification-item-body">
                  ...
                  <div className="notification-meta-row">
                    ...
                    {date && <span>{date}</span>}
                    <span
                      className={`notification-read-status ${notification.dataLeitura ? 'is-read' : 'is-unread'}`}
                    >
