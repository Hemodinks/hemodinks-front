...
describe('App medical access', () => {
  ...
  it('exibe pacientes para medico mesmo sem feature explicita de pacientes na licenca', async () => {
    const { user } = await renderAuthenticatedApp({
      sessionOverrides: {
        ...
        perfilNome: 'Medicos',
        nome: 'Dra. Ana',
        licenca: buildMedicalLicense(['Dashboard.Visualizar', 'Cbhpm.Consultar']),
      },
    });
  ...
  it('separa consulta CBHPM do cadastro de paciente para medico', async () => {
    ...
    expect(
      screen.getByText(/dados clínicos, procedimentos e valores são registrados no atendimento/i),
    ).toBeInTheDocument();
  });
  ...
  it('permite ao Super Administrador editar o próprio cadastro com todos os perfis e confirma a troca de perfil', async () => {
    ...
    expect(
      within(profileSelect).getByRole('option', { name: 'Administrador' }),
    ).toBeInTheDocument();
    ...
    expect(
      within(profileSelect).getByRole('option', { name: 'SuperAdministrador' }),
    ).toBeInTheDocument();
