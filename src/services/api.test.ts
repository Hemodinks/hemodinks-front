...
describe("services api client", () => {
  ...
  it("monta os payloads de CRUD de pacientes", async () => {
    ...
    requestSpy
      .mockResolvedValueOnce(axiosResponse([]))
      .mockResolvedValueOnce(
        axiosResponse([{ id: 1, nome: "Santa Clara - Mater Dei" }]),
      )
      .mockResolvedValueOnce(
        axiosResponse([{ idConvenio: 7, descricaoConvenio: "Particular" }]),
      )
      .mockResolvedValueOnce(
        axiosResponse([{ idFornecedor: 1, fornecedor: "Promedom" }]),
      )
      .mockResolvedValueOnce(axiosResponse({ id: 10 }))
      .mockResolvedValueOnce(axiosResponse({ id: 10 }))
      .mockResolvedValueOnce(
        axiosResponse({ id: 1, nomeOriginal: "laudo.pdf" }),
      )
    ...
    await expect(
      deletePacienteArquivo(10, 1, "jwt-token"),
  ...
  it("carrega todas as paginas da consulta CBHPM para cache local", async () => {
    ...
    expect(requestSpy.mock.calls[1]?.[0]?.params?.toString()).toBe(
      "page=2&pageSize=100",
    );
  });
  ...
  it("resolve slug de clinica a partir de hostname customizado", () => {
    expect(resolveClinicaSlugFromHostname("clinica-alfa.hemodinks.com")).toBe(
      "clinica-alfa",
    );
    expect(
      resolveClinicaSlugFromHostname(
        "hemodinks-front-confirmation.onrender.com",
      ),
    ).toBeNull();
  ...
  it("baixa documentos privados com token bearer", async () => {
    ...
    await expect(downloadPacienteArquivo(10, 3, "jwt-token")).resolves.toBe(
      patientBlob,
    );
    await expect(downloadUserArquivo(2, 7, "jwt-token")).resolves.toBe(
      userBlob,
    );
