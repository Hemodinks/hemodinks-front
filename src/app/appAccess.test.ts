...
describe('matriz de acesso da interface por perfil', () => {
  ...
  it.each(cases)('%s aplica aos relatórios a mesma autorização da gestão de faturamento', (_name, perfilId) => {
    const access = getAppAccess(mockSession({ perfilId }));
    expect(access.canAccessReports).toBe(access.canAccessBilling);
    expect(access.canUseReportsRoute).toBe(access.canUseBillingRoute);
  });
});
