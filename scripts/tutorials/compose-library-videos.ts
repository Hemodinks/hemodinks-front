...
type Manifest = { steps: Array<{ id: string; text: string; audioFile: string }> };
...
const timelines = await findFiles(resultRoot, 'timeline.json');
...
for (const timelineFile of timelines) {
  ...
  const manifest = JSON.parse(await readFile(join(root, 'audio-manifest.json'), 'utf8')) as Manifest;
  const steps = manifest.steps.map((audio, index) => ({ ...audio, ...timeline.steps[index] }));
  if (steps.some((step) => !step.id || step.id !== manifest.steps[steps.indexOf(step)].id)) throw new Error(`Timeline divergente: ${timeline.tutorialId}`);
